...
/* Brand lockup bottom-left, from the same master as the site header
   (brand/logo.png, see scripts/build-brand.mjs), small enough to stay out of
   the artwork's way. */
const wordmark = await sharp(path.join(ROOT, 'brand', 'logo.png'))
  .trim({ threshold: 1 })
  .resize({ height: 52 })
  .png()
  .toBuffer();
